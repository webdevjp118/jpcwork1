...
function cw_post_type_magazine() {
    ...
    $labels = array(
        'name' => _x('magazine', 'plural'),
        'singular_name' => _x('magazine', 'singular'),
        'menu_name' => _x('Magazine', 'admin menu'),
        'name_admin_bar' => _x('magazine', 'admin bar'),
        'add_new' => _x('Add New', 'add new'),
        'add_new_item' => __('Add New Magazine'),
...
function laundry_ikitai_it() {
	...
	$ikitais = get_post_meta( $laundry_id, 'laundry_ikitai_count', true );
	$ikitais = ( empty( $ikitais ) ) ? 0 : $ikitais;

 	$user_ikitais = get_user_meta($user_id, 'user_ikitai_list', true);
	if($user_ikitais) {
		$user_ikitais = json_decode($user_ikitais);
	} else {
		$user_ikitais = [];
	}
	
	if(in_array($laundry_id, $user_ikitais)) {
		$new_ikitais = $ikitais - 1;
		$pos = array_search($laundry_id, $user_ikitais);
		unset($user_ikitais[$pos]);
	}
	else {
		$new_ikitais = $ikitais + 1;
		array_push($user_ikitais, $laundry_id);
	}
 	
	$user_ikitais = json_encode($user_ikitais);
	update_post_meta($laundry_id, 'laundry_ikitai_count', $new_ikitais );
	update_user_meta($user_id, 'user_ikitai_list', $user_ikitais );
