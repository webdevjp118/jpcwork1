...
function date_to_input($str) {
  if(empty($str)) $str = date('Y/m/d');
  $date = strtotime( $str );
  return date('Y', $date).'年'.date('m', $date).'月'.date('d', $date).'日';
}
function input_to_date($strdate) {
  if(empty($strdate)) $strdate = date('Y/m/d');
  $strdate = str_replace('年','/',$strdate);
  $strdate = str_replace('月','/',$strdate);
...
else $user_birthday = date_to_input(get_user_meta($user_id, 'birthday', true));
...
if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'update_user' ) {
    ...
    if ( !empty( $_POST['birthday'] ) ) update_user_meta($user_data->ID, 'birthday', input_to_date($_POST['birthday']) );
...
<table class="c-formTable">
    <tbody>
        <tr>
          <th class="c-formTable_th">コイラン歴</th>
          <td class="c-formTable_td">
        ...
        <tr>
          <th class="c-formTable_th">好きなコイラン</th>
          <td class="c-formTable_td">
            <div class="c-formTextarea">
              <textarea placeholder="例：布団乾燥ができるコイラン。Wi-Fi あり、カフェ併設。" name="favor_laundry" cols="50" rows="10"></textarea>
            </div>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td colspan="2" class="c-formTable_td">
            <div class="c-formText">
              <input placeholder="https://blog.coirango.com" name="web_url" type="text" value="<?php echo $user_weburl; ?>">
            </div>
          </td>
        </tr>
      </tbody>
