...
$magazine_recent = new WP_Query($search_recent);
$total_count = $magazine_query->found_posts;
$posts_per_page = 9;
?>
...
<?php
    if($magazine_recent->have_posts()) : 
        $magazine_recent->the_post();
...
?>
        <div class="latestpost-sec">
            <div class="latest-post">
                <div class="js-button wrap" data-href="<?php echo get_post_permalink(); ?>">
                    <div class="photo-div">
...
<?php
    ...
    if($pageno == 1) {
        if($magazine_query->have_posts()) $magazine_query->the_post();
    }
?>
        <div class="post-list">
<?php
...
<?php
	...
	if($total_count > 0) {
		...
		if($page_first > 0) echo '<a class="page-other" href="'.get_site_url().'/magazines/?pageno='.$page_first.'">'.$page_first.'</a>';
		if($page_prev > 0) echo '<a class="page-other" href="'.get_site_url().'/magazines/?pageno='.$page_prev.'">'.$page_prev.'</a>';
		if($pageno > 0) echo '<a class="page-current" href="'.get_site_url().'/magazines/?pageno='.$pageno.'">'.$pageno.'</a>';
		if($page_next > 0) echo '<a class="page-other" href="'.get_site_url().'/magazines/?pageno='.$page_next.'">'.$page_next.'</a>';
		if($page_last > 0) echo '<a class="page-other" href="'.get_site_url().'/magazines/?pageno='.$page_last.'">'.$page_last.'</a>';
		echo '</div>';
    }
?>
    </div><!-- c-container -->
